...
const Renderer = (() => {
  ...
  function drawSky(ctx,W,H,frame){
    ...
    const mpts1=[0,.17,.09,.07,.19,.14,.32,.03,.44,.12,.56,.06,.67,.13,.77,.04,.87,.11,.95,.08,1,.16];
    for(let i=0;i<mpts1.length;i+=2){
      ctx.lineTo(mpts1[i]*W, hY-mpts1[i+1]*hY*1.12);
    }
    ctx.lineTo(W,hY); ctx.closePath(); ctx.fill();

    ctx.fillStyle='rgba(8,5,18,.92)';
    ctx.beginPath(); ctx.moveTo(0,hY);
    const mpts2=[0,.08,.14,.03,.27,.09,.39,.02,.52,.07,.64,.03,.75,.08,.87,.02,1,.07];
    for(let i=0;i<mpts2.length;i+=2){
      ctx.lineTo(mpts2[i]*W, hY-mpts2[i+1]*hY*.72);
    }
    ctx.lineTo(W,hY); ctx.closePath(); ctx.fill();
  }
